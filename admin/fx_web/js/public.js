...
//接口地址
var baseUrl = 'http://api.mycxf.com';   //http://api.mycxf.com
...
var requestUrl = {
    ...
    'check_is_bindtaobao':baseUrl+'/check_is_bindtaobao',    //是否绑定淘宝店铺
    'goods_center_two':baseUrl+'/goods_center_two'   //货源中心 二级类目
};
...
function taobao_auth() {
    ...
    if (confirm("绑定淘宝店铺，您将绑定创想范货源分销平台的应用，是否继续？")) {
        window.open("https://oauth.taobao.com/authorize?spm=a219a.7386781.0.0.qOgstw&response_type=code&client_id=23302533&redirect_uri=http://cxf2016.hz.taeapp.com&state=2366&view=web#");
...
function openTB() {
    ...
    var url = 'https://oauth.taobao.com/authorize?spm=a219a.7386781.0.0.qOgstw&response_type=code&client_id=23302533&redirect_uri=http://cxf2016.hz.taeapp.com&state='+userID+'&view=web#';
    X.Post(requestUrl.check_is_bindtaobao, 1,{user_id:userID}, function (res) {
        if(res.header.stats==0){
            if(res.body.list.is_bind==0){
                window.open(url,'店铺绑定','width=800,height=500,top=200,left=500');
                $('.PH').hide();
            }else {
                if(JSON.parse(unescape(data)).length>=5){
