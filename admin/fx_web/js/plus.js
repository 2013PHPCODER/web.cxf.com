...
var X = {
    ...
    bindModel: function (url, type, data, pos, arr, callback, callback1) {
		...
		//              $('#' + arr[i]).hide();
		              $('#' + arr[i]).css({'opacity':0,'filter':'alpha(opacity=0)'});
		          }
		      }
        }
        var self = {};
        ...
        $.post(url, JSON.stringify(self.data), function (data) {
            if (arr[0] == 'category' || arr[0] == 'searchCatage') {          //特殊接口这里要定义

            } else {
                if (data.header.stats != 0) {
                    for (var i in arr) {
                        if(!$('#'+arr[i]).hasClass('marks')){
                            $('#' + arr[i]).css({'opacity':1,'filter':'alpha(opacity=100)'});
                            $('#'+arr[i]).html('');
                        }
                    }
                    //X.notice('没找到数据哦', 3);
