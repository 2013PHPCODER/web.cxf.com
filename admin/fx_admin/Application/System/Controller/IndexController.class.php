...
class IndexController extends Auth{			//管理员首页
	...
	public function index(){			//显示管理员操作信息
		...
		$condition['a.admin_user_id']=session('user.id');
		...
		foreach ($r['list'] as &$v) {			//数据处理
			...
			$v['module']=Config('MODULE_NAME.'. strtolower($v['module']) );
		}
		...
		// dump($_SESSION['auth']);
		// dump($_SESSION['auth_show']);return;
		$this->display();

	}

	public function showAuth(){				//显示我的权限
		$this->assign('auth', showAdminSelfAuth(session('auth')));
		$this->display();
	}
	...
	public function edit(){					//管理员密码修改
		...
		$r=M('fx_admin_user')->where($condition)->field('admin_user_id, pwd')->find();
		if (!$r || !verifyPwd($old, $r['pwd'])) {			//检查密码是否正确
			$this->ajaxReturn(\status::error('当前登录密码错误'));
			return;	
		}								
		$condition['admin_user_id']=session('user.id');
		$data=['pwd'=>encodePwd($pw1), 'update_time'=>time()];
