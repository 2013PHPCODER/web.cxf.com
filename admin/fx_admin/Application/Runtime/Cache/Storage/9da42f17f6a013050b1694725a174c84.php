...
<!--                         <li data-cate="pageManage" >
    ...
    <div class="box box-warning">
        <div class="box-header with-border">
            <div class="box-title">
                <ul class="choose_ul">
                    <li><a class="<?php echo xeq(I('get.group_id',1),1,'btn btn-warning btn-xs');?>" href="<?php echo U('storage/storageManger',array('group_id' => 1 ));?>" >所有商品</a></li>
                    <li><a class="<?php echo xeq(I('get.group_id'),2,'btn btn-warning btn-xs');?>" href="<?php echo U('storage/storageManger',array('group_id' => 2 ));?>">预警商品</a></li>
                </ul>
            </div>
        ...
        <div class="box-body">
            <div class="row" >
                <form class="form-inline" method="get" action="<?php echo U('storage/storageManger');?>" id="searchForm">
                    ...
                    <div class="row" >
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control input-xs" value="<?php echo I('get.startTime');?>" id="startTime" name="startTime" placeholder="开始时间">
                        </div>
                        <div class="form-group">
        ...
        <div class="box-body table-responsive no-padding">
            <table class="table table-hover" id ="dataTable">
                ...
                <?php if(is_array($datas["list"])): $i = 0; $__LIST__ = $datas["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="content">
                    ...
                    <td><?php if(is_array($vo["sku_str_zh"])): foreach($vo["sku_str_zh"] as $key=>$item): ?><P> <?php echo ($item); ?></P><?php endforeach; endif; ?></td>
                    <td>
                        <?php foreach($vo['stock_num'] as $key =>$value){ ?>
                        <?php if($value['stock_num'] <= $vo['stock_lock_num'][$key] and $vo['stock_lock_num'][$key] > 0 ){ ?>
                    ...
                    <td ><?php if(is_array($vo["stock_lock_num"])): foreach($vo["stock_lock_num"] as $key=>$item): ?><input type="text" class="form-control input-xs edit_input"  disabled="disabled" data-skuid="<?php echo ($value["id"]); ?>"  value="<?php echo ($item); ?>"><?php endforeach; endif; ?></td>
                    <td><?php echo ($vo["all_stock"]); ?></td>
            ...
            $('.all_edit').click(function () {
                if ($('.choose:checked').serialize() || $("#selectall").is(':checked')) {
                    ...
                    layer.confirm(str, {title: "批量修改库存", btn: ['确定', '取消']}, function (index) {
                        ...
                        var urlPost = "<?php echo U('storage/allEdit',I('get.'));?>";
                        $.post(urlPost, data, function (result) {
                            if (result.status == 'ok') {
                                layer.msg('修改成功');
                            } else {
