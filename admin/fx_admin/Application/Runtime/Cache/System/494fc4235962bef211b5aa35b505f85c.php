        ...
        <div class="row" >
            <form class="form-inline" method="get" action="<?php echo U(ACTION_NAME, I('get.'));?>" id="searchForm">
                ...
                <div class="form-group">
                    ...
                    <input name="sensitive" value="<?php echo I('get.sensitive');?>" type="text" />
                </div>
                ...
                <!--div class="form-group">
                    ...
                    <select class="form-control input-xs" name="type">
                        ...
                        <option value="政治" <?php echo xeq(I('get.type'),'政治','selected');?>>政治</option>
                        <option value="暴力" <?php echo xeq(I('get.type'),'暴力','selected');?>>暴力</option>
                        <option value="涉黄" <?php echo xeq(I('get.type'),'涉黄','selected');?>>涉黄</option>
                        <option value="赌博" <?php echo xeq(I('get.type'),'赌博','selected');?>>赌博</option>
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control input-xs" name="grade">
                        <option value="">请选择</option>
                        <option value="1" <?php echo xeq(I('get.grade'),1,'selected');?>>1</option>
                        <option value="2" <?php echo xeq(I('get.grade'),2,'selected');?>>2</option>
                        <option value="3" <?php echo xeq(I('get.grade'),3,'selected');?>>3</option>
                    </select>
                </div-->
                <div class="form-group">
        ...
        <table class="table table-hover">
            ...
            <?php if(empty($datas)): ?><tr><td colspan="6" style="text-align: center;"><font color="red">暂无数据</font></td></tr>
                ...
                <?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($vo["sensitive"]); ?></td>
                        <!--td><?php echo ($vo["type"]); ?></td>
                        <td><?php echo ($vo["grade"]); ?></td-->
                        <td><?php echo ($vo["name"]); ?></td>
                        <td><?php echo (date('Y-m-d H:i:s',$vo["datetime"])); ?></td>
                        <td data-id="<?php echo ($vo["id"]); ?>"><a href="<?php echo U('system/pageManage/editSensitive', ['id'=>$vo['id']]);?>">编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;
...
<div class="dalog-modal" id="aleatMoudle">
    ...
    <div class="g-modal-content">
        <form class="form-inline" method="post" action="<?php echo U('system/pageManage/addSensitive');?>">
            <div class="row">
    ...
    $('.delete_data').click(function () {
        ...
        layer.confirm('你确定删除吗？', {btn: ['确认', '取消']},
                function (index) {
                    layer.close(index);
                    $.ajax({
                        type: 'post', url: "<?php echo U('system/PageManage/delSensitive');?>", data: {id: id},
                        success: function (e) {
                            X.notice(e.info, 3);
