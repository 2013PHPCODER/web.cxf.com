...
class OrderController extends BasicController {
    ...
    public function index() {
        ...
        $this->datas = $this->getOrderList($this->searchWhere(), I('get.sort') ? str_replace('~', ' ', I('get.sort')) : 'order_list.order_id desc');
...
//            array('concat','收货地址'),
        ...
        $sql = 'SELECT ol.order_id,og.goods_id,ol.order_sn,ol.add_time,ss.shop_name,ol.buyer_name,ol.qq,ol.is_cus,og.goods_name,
                og.buyer_goods_no,ogs.sku_comb_id,ol.memo,og.goods_num,
                ol.goods_amount,ol.order_amount,ol.shipping_fee,oc.*,
                ol.order_state,(SELECT distribution_price FROM goods_price WHERE goods_id=og.goods_id AND shop_id=ol.shop_id limit 1) as `price`
                 FROM order_list as `ol`, order_goods as `og`,system_shop as `ss` ,
                order_goods_sku as `ogs`,order_contact as `oc` 
                 WHERE `ol`.order_id=`og`.order_id and ol.shop_id=ss.shop_id 
        ...
        foreach ($data as $key => $val) {
            ...
            $sku = $this->getOrderGoodsSK($val['order_id'], $val['goods_id']);
            $sku_str = '';
            foreach ($sku as $k => $v) {
                $sku_str .= $k . ':' . $v . '  ';
            }
            $_data[$key]['sku'] = $sku_str;
...
//        }
    ...
    public function edit_address() {
        ...
        $data['dist'] = $data['dist'] ? M('area_list')->where("id = " . $data['dist'])->getField('area_name') : '';
     ...
     * @param  string $mOrder [订单牌勋]
     * @return [Array]         [订单列表数组]
     */
    public function getOrderList($mWhere = '', $mOrder = '', $type = '') {
        $_count = M('order_list')->join('order_goods ON order_goods.order_id = order_list.order_id')
                        ->join('goods_list ON goods_list.goods_id = order_goods.goods_id')
                        ->where($mWhere)->count();
        $_page = $this->getPage($_count);
        $order_list = M('order_list');

        if (!empty($type)) {
            $order_list->join('order_goods ON order_goods.order_id = order_list.order_id');
            $_list = $order_list->Field('order_list.order_id')->where($mWhere)->order($mOrder)->select();
            foreach ($_list as $key => $value) {
                $_data['list'][] = $value['order_id'];
            }
        } else {
            $order_list->join('order_goods ON order_goods.order_id = order_list.order_id');
            $order_list->join('goods_list ON goods_list.goods_id = order_goods.goods_id');
            $order_list->field('order_list.*,order_goods.*,goods_list.*');
            $_data['list'] = $order_list->where($mWhere)->order($mOrder)->limit($_page->firstRow . ',' . $_page->listRows)->select();
            foreach ($_data['list'] as $k => $v) {
                ...
                $_data['list'][$k]['img_path'] = $this->getOrderImg($v['order_id'])['img_path'];
                $_data['list'][$k]['goods_price'] = $this->getDistributionPrice($v['goods_id'], $v['shop_id']);
                $message = M('order_message')->where('order_id =' . $v['order_id'])->getField('message');
                if (!empty($message)) {
                    $_data['list'][$k]['message'] = 1;
                } else {
                    $_data['list'][$k]['message'] = 0;
                }
            }
    ...
    public function getOrderImg($mOrderId) {
        ...
        $img_path = $order_goods->where("order_goods.order_id ={$mOrderId}")->find();
        return $img_path;
    }
