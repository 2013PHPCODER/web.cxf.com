...
class OrdersController extends CommonController {
    ...
    public function searchWhere() {
        $_where['supplier_id'] = session('user_info')['id'];
        //订单状态
        //所有订单
        if (1 == I('get.group_id') || empty(I('get.group_id'))) {
            $_where_t['order_state'] = 5;
            $_where_t['is_supplier_close'] = 0;
            $_where_t['_logic'] = 'and';
            $_where_t_1['_complex'] = $_where_t;
            $_where_t_1['order_state'] = array('in', array(1, 2, 3, 4, 6));
            $_where_t_1['_logic'] = 'or';
            $_where['_complex'] = $_where_t_1;
        }
        //待确认
        if (2 == I('get.group_id')) {
    ...
    public function orderDetail() {
        ...
        $fields = "order_list.order_id,order_list.order_sn,order_list.add_time,order_list.shop_id,order_list.buyer_name,order_list.qq,order_list.is_cus,"
                . "order_list.memo,order_list.order_state,order_list.shipping_fee,order_list.cost_price pay_amount,order_list.message_starts,shipping_code,"
...
//            $goods_info = M('goods_list')->where("goods_id=" . $vv['goods_id'])->find();
            ...
            if (!empty($vv['sku_comb_id'])) {
                $vv['price'] = $vv['price'];
                $vv['goods_price_total'] = bcmul($vv['price'], $vv['goods_num'], 2);
        ...
        //订单的价格
        $datas['order_goods_price'] = $order_amount;
        /* 获取最新的2条收货人信息 */
...
//        $datas['shop_id'] = M('system_shop')->where("shop_id=" . $datas['shop_id'])->getField('shop_name');
    ...
    public function Message() {
        ...
        if (I('post.order_id')) {
            ...
            $this->result = M('order_message')->where("order_id={$this->order_id} and user_type<>3 and to_user_type<>3")
    ...
    public function saveMessage() {
        if (I('post.')) {
            ...
            $message->user_type = 1;
            $message->to_user_type = 2;
            $message->user_id = $this->user_info['id'];
