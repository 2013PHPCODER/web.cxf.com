 ...
 */
function uploadKey($filename, $type = '') {
    $options = array();
    $policy = array();
    $signature = array();
    $c=C('IMG');
    ...
    switch ($type) {
        case 'goods':
            $path = $c['upload']['path']['goods'];
            break;
        case 'ad':
            $path = $c['upload']['path']['ad'];
            break;
        default:
            $path = $c['upload']['path']['other'];
            break;
    }
    $len = count($filename);

    for ($i = 0; $i < $len; $i++) {
        $options[$i] = ['bucket' => $bucket, 'save-key' => $path . $filename[$i], 'expiration' => time() + $c['upload']['valid_time']];           //上传有效期
...
function f_afterOperator($status, $type='refund'){                   //根据状态值生成售后列表不同的操作（html代码）
    if ($type=='refund') {
        $ref=new ReflectionClass('aftersale_back_money_status');
    }else{
        $ref=new ReflectionClass('aftersale_back_good_status');
