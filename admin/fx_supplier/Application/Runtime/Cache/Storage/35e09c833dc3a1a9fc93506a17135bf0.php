        ...
        <div class="box-tools">
            <form method="get" action="<?php echo U(ACTION_NAME,I('get.'));?>" id="sortForm">
                <div class="input-group order">
                    <ul>
                        <?php switch($_GET['group_id']): case "1": ?><li><a href="#" class="btn btn-default btn-xs" id="exportOrderExecl">导出</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs cancel_assign">取消分配</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs print_distribution">打印配货单</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs print_ship">打印物流单</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs distribution">配货</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs assign">分配</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs send_goods">发货</a></li><?php break;?>
                        <?php case "2": ?><li><a href="javascript:" class="btn btn-default btn-xs print_distribution">打印配货单</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs distribution">配货</a></li><?php break;?>
                        <?php case "3": ?><li><a href="javascript:" class="btn btn-default btn-xs assign">分配</a></li><?php break;?>
                        <?php case "4": ?><li><a href="javascript:" class="btn btn-default btn-xs cancel_assign">取消分配</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs print_ship">打印物流单</a></li>
                            <li><a href="javascript:" class="btn btn-default btn-xs send_goods">发货</a></li><?php break;?>
                        <?php case "5": ?><li><a href="javascript:" class="btn btn-default btn-xs print_ship">打印物流单</a></li><?php break;?>
                        <?php default: ?>
                        ...
                        <li><a href="javascript:" class="btn btn-default btn-xs distribution">配货</a></li>
                        <li><a href="javascript:" class="btn btn-default btn-xs assign">分配</a></li>
                        <li><a href="javascript:" class="btn btn-default btn-xs send_goods">发货</a></li><?php endswitch;?>
                    </ul>
                </div>
...
<form action="<?php echo U('storage/exportOrderExcel');?>" id="exportOrderExeclForm" method="post">
    <input id="export_order_id" name="order" type="hidden">
</form>
    ...
    <script type="text/javascript">
        ...
        var checkTaobaoLogin = function () {
            $.post("<?php echo U('TbApi/checkAcess');?>", function (data, textStatus, xhr) {
                ...
                if (0 == data.status) {
                    // var str = $("<script src='https://login.taobao.com/member/login.jhtml'><\/script>");
                    // $("body").append(str);
                    // setTimeout(function(){
                    // },5000);
                    $('#getcode').submit();
...
//发货事件
        var sendGoods = function (val, num) {
        ...
        $(document).ready(function () {
                            ...
                            function () {
                                $.post("<?php echo U('storage/resultDistribution');?>", $('.print_table').serialize(), function (status) {
                                    ...
                                    newOpenWindow.onload = function () {
                                        newOpenWindow.document.body.innerHTML = newstr;
                                    };
                                    newOpenWindow.window.print();
                                });
                            });
                });
                return false;
            ...
            $('.print_ship').click(function () {
                if (false == $("#selectall").is(':checked') && 0 == $('.choose:checked').length) {
                    layer.alert('请选择要print_ship操作的发货单');
                    return false;
                }
                var falgs = true;
                var dataPost = new Object(),
                ...
                $.post(urlPost, dataPost, function (result) {
                    layer.confirm(result, {title: '打印物流单', btn: ['全选', '打印', '取消'], area: ['1100px', '600px']}, function () {

                        falgs ? function () {
                            $('#printData td input[type=checkbox]').prop('checked', true);
                            falgs = false;
                        }() : function () {
                            $('#printData td input[type=checkbox]').prop('checked', false);
                            falgs = true;
                        }()

                    }, function () {
                        if ($('.print:checked').length == 0) {
                            layer.tips('请选择要操作的发货单', '.layui-layer-btn1', {tips: [4, '#3595CC']});
                            return false;
                        }
                        var postData = new Object(), order = new Array();
                        var _class = $('.print:checked').first().attr('class');
                        var flag = true;

                        $('.print:checked').each(function (index, el) {
                            if (_class != $(this).attr('class')) {
                                layer.tips('请选择同一种快递面单类型', '.layui-layer-btn1', {tips: [4, '#3595CC']});
                                flag = false;
                                return false;
                            }
                            return false;
                        ...
                        if ($(".print:checked").hasClass("c_ship")) {
                            ...
                            $.post(postUrl, postData, function (data, textStatus, xhr) {
                                layer.confirm(data, {title: "打印物流单  <a href=<?php echo U('getParseXML');?> target='_blank'>设置模板</a>", btn: ['打印', '取消'], move: false, area: ['1000px', '600px']}, function (index) {
...
//					layer.close();
            ...
            var printShip = function (shipData) {
                ...
                for (var i = 0; i < shipData.length; i++) {
                    ...
                    CNPrint.SET_PRINT_CONTENT("ali_waybill_consignee_address", shipData[i].shipping_info.trade_order_info.consignee_address.province + shipData[i].shipping_info.trade_order_info.consignee_address.city + shipData[i].shipping_info.trade_order_info.consignee_address.area + shipData[i].shipping_info.trade_order_info.consignee_address.address_detail);
            ...
            $('.send_goods').click(function () {
                var str = "<div class='row'> <div class='form-group' style='margin-right: 0'>";
                str += "<label style='display: block'>运单号:</label><input style='display: inline-block' type='text' name='assign' id='inputEnter'  class='form-control input-xs' value=''><span style='margin-left: 12px'>累计：<b id='contentNumber'>0</b></span>";
                str += "</div></div>";
                layer.confirm(str, {title: '提醒', btn: ['确定', '取消'], area: ['300px', '220px']}, function () {
                    var val = $('#inputEnter').val().replace(/\s/ig, '');
                    if (val) {
                        $('#inputEnter').val(val.replace(/\s/ig, ''));
                        var num = parseInt($('#contentNumber').text());
                        sendGoods(val, num);
                    } else {
                        $('#contentNumber').text(0);
                        layer.close();
                        window.location.reload();
                    }
                });
                $('#inputEnter').keydown(function (event) {
                    ...
                    if (e.keyCode == 13) {
                        sendGoods($(this).val().replace(/\s/ig, ''));
                    }
            ...
            $('.excep').click(function () {
                ...
                layer.confirm('点击确认后,进入异常状态,请确认', {title: '异常操作', btn: ['确认', '取消']}, function (index) {
                    $.post("<?php echo U('excepOrder');?>", postData, function (result) {
