...
class BaseModel {
    ...
    function __construct($tablename) {
        $this->table = Config('db.prefix') . $tablename;
    }
    ...
    final public function valid() {
        ...
        unset($obj_value['_cxf_num_id']);
        foreach ($obj_value as $key => $value) {
            $type = static::$init_valid_array[$key][0];
            $len = static::$init_valid_array[$key][1];
            $isnull = static::$init_valid_array[$key][2];
            if ($type == "int") {
