 ...
 ** by XuJing(2016/6/13)  499190982@qq.com  **
        ...
        handler.mousedown(function (e) {
	        ...
	        $(document).mousemove(function(e){
	            ...
	            if(isMove){
	        	  ...
	        	  if(isMove){
			        	  if(parseInt(handler.css('left')) >= maxWidth-10){  //鼠标指针移动距离达到最大时清空事件
	            ...
	            isMove = false;
	            var _x = e.pageX - x;
	            if(parseInt(handler.css('left')) < maxWidth-10){ //鼠标松开时，如果没有达到最大距离位置，滑块就返回初始位置
	                handler.css({'left': 0});
	                drag_bg.css({'width': 0});
	            }
	        });
...
//    验证为空
    ...
    mes_center: function () {
        var div = document.createElement('div');
        $(div).addClass('marksT');
        $(div).addClass('Agmarks');
        $(div).css('display', 'block');
        var id = localStorage.getItem('user_id');
        if (id == '' || id == null) {
            X.notice('您尚未登录', 3);
        } else {
            X.Post(requestUrl.topNotice, 1, {'to_client': 2, 'page': 1}, function (e) {
                var html = '<div class="PopDiv mes-center" style="display: block;">'
                        + '<div class="PopHeader">'
                        + '<img src="images/main/messalert.png" alt="">'
                        + '<span class="PopTitle">消息中心</span>'
                        + ' <div class="PopColse"></div>'
                        + ' </div>'
                        + ' <div class="PopBody center-PopBody">'
                        + '	<div class="Phcontent">'
                        + '	<div class="PhTab AGmes-table">'
                        + '       <table style="width: 100%;">'
                        + '          <tr>'
                        + '    <th>发布时间</th>'
                        + '     <th style="border-left: none;">内 容</th></tr>';
                var newDate = new Date();
                if (e && e.body && e.body.list && e.body.list.list) {
                    var b = e.body.list.list;
                    for (var i in b) {
                        newDate.setTime(b[i].addtime * 1000);
                        var time = newDate.toLocaleString();
                        html += '<tr><td>' + time + '</td><td>' + b[i].title + '</td></tr>'
                    }
                }
                html += ' </table></div></div></div></div>';
                div.innerHTML = html;
                document.body.appendChild(div);
                $(div).find('.PopColse').on('click', function () {
                    document.body.removeChild(div);
                });
            })
        }
    ...
    vipUp: function (callback) {
        ...
        var html = '<div class="PopDiv mes-center" style="display: block;">'
                + '<div class="PopHeader">'
                + '<img src="images/markicon03.png" alt="">'
                + '<span class="PopTitle">账户升级</span>'
                + ' <div class="PopColse"></div>'
                + ' </div>'
                + '<div class="PopBody">'
                ...
                + '		<span>当前版本：</span>'
                + '	<strong><i class="price-icon"><img src="images/level01.png"/></i>初级版</strong>'
                + '	</div>'
                ...
                + '	</select>'
                + '	<span class="price">版本价格:<i>200.00元</i></span>'
                + '	</div>'
                + '	<div class="cont-box-text bg-color">'
                + '		<span>升级费用：</span>'
                ...
                + '</div><div class="cont-box-text"><p><input type="button" class="btn btn-up" id="vipConfirm" value="提交" /></p></div></div></div></div> ';
        div.innerHTML = html;
        document.body.appendChild(div);
        $('#drag').drag();
        if (callback) {
            $('#vipConfirm').on('click', function () {
                callback();
            })
        }
        $(div).find('.PopColse').on('click', function () {
...
//										$('#'+files[i].id).height($('#'+files[i].id).width()*3/4);	//固定高宽比
            ...
            X.Post(requestUrl.get_img_token, 1, datar, function (data) {
